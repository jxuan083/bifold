...
| `.tex` | Tectonic 編出的 PDF | **點 PDF 上的任一行就跳到原始碼**，跨檔會自動切過去；游標移動時 PDF 跟著捲；編譯錯誤直接標行號 |

## 用法

```bash
npm install                       # 只有一個依賴：pdfjs-dist
node src/server.js 8790 /abs/path/to/deck.html
node src/server.js 8790 /abs/path/to/paper/main.tex
```

啟動後開 http://127.0.0.1:8790。左上角 ☰ 開關檔案樹，點檔案就切過去。要換專案可以按「檔案…」「資料夾…」叫出 Finder，或直接貼絕對路徑。
...
`.tex` 模式下選取模式、屬性面板、PPTX 匯出都會隱藏——那些是針對 HTML 的 DOM 操作，PDF 上沒有對應物。

SyncTeX 回報的行號有時是段落結尾的空行而不是你點的那一句，這是 TeX 的行為（整段排版歸給 `\par` 所在的位置），不是查詢查錯。
